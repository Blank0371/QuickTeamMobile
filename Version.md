...
### 1.1.16
- **Abmelden räumt das Gerät auf.** Vorher blieb nach dem Abmelden alles Personenbezogene des bisherigen Nutzers auf dem Gerät: der Push-Token hing weiter an seinem Konto (auf einem geteilten Gerät kamen seine Pushes also weiter an), der Offline-Dienstplan (`cache:`-Schlüssel) war für die nächste Person offline lesbar, seine Benachrichtigungs-Abwahlen (`notifPrefs`) galten für den Nächsten weiter, und die lokal geplanten Schicht-Erinnerungen feuerten weiter. Jetzt: `signOut()` ruft **vor** dem Abmelden `push_token_loeschen()` auf (braucht `auth.uid()`; der zuletzt registrierte Token liegt dafür unter `pushToken` in AsyncStorage, best-effort, höchstens 5 s — offline blockiert das Abmelden nicht, die nächste Anmeldung auf dem Gerät hängt den Token ohnehin um). Bei jedem echten `SIGNED_OUT` (Knopf, Kontolöschung, gelöschtes Konto, ungültiger Refresh-Token) werden `cache:`-Einträge und `notifPrefs` gelöscht und alle geplanten Erinnerungen storniert — **nicht** beim Kaltstart ohne Sitzung und nicht offline, wo der Cache gerade gebraucht wird. Handover-Punkte aus `QuickTeamFront/docs/review-2026-09-14.md` §5. — `src/context/auth.tsx`, `src/lib/cache.ts`, `src/lib/notifications.ts`

### 1.1.17
- **`push-versenden` entfernt tote Push-Token.** Meldet Expo im Push-Ticket `DeviceNotRegistered` (App deinstalliert, Token rotiert), wird der Token aus `push_tokens` gelöscht, statt bei jeder Benachrichtigung erneut angeschrieben zu werden (Expo-Doku: „stop sending messages to the corresponding Expo push token"). Tickets kommen in derselben Reihenfolge wie die Nachrichten zurück. **Grenze:** derselbe Fehler kann auch erst in den Push-*Receipts* auftauchen; die werden weiterhin nicht abgefragt. Antwort enthält zusätzlich `removed`. **Deployed am 2026-09-14 als Version 4** (`verify_jwt` weiterhin aus; Rauchtest ohne Secret → 403). — `supabase/functions/push-versenden/index.ts`

### 1.1.18
- **Launch-Checkliste Punkt 10 — `plan-generieren` serverseitig gedrosselt** (Spezifikation `QuickTeamFront/docs/audit-a/app-entwickler-handover.md` §5, Parameter von der Betreiberin am 2026-09-11 entschieden). (1) **Sperrzeit 30 s pro Betrieb** über alle Zyklen, gemessen ab dem letzten Start oder Ende eines Laufs → `429` mit `code: "sperrzeit"` und `retry_after_s`. (2) **Kein zweiter Lauf auf demselben Zyklus:** der Wechsel auf `solver_laeuft` ist jetzt ein atomarer Anspruch (UPDATE nur, solange `status` und `solver_gestartet_am` noch dem gelesenen Stand entsprechen; der zweite gleichzeitige Aufruf trifft keine Zeile) → `409` mit `code: "laeuft"`. Ein laufender Zyklus wird auch mit `{ force: true }` nicht übernommen — nur ein verwaister (`solver_laeuft` älter als 10 min, dieselbe Schwelle wie `LAUF_GEDULD_MINUTEN` im Web-Dashboard); scheitert dieser Lauf, fällt der Zyklus auf `offen` statt zurück auf `solver_laeuft`. Vorher war der Solver unbegrenzt oft aufrufbar, und zwei gleichzeitige Aufrufe auf denselben Zyklus liefen beide. App: `doGenerate` wertet den Aufruf jetzt aus und zeigt bei Fehler „Zeitraum angelegt – die automatische Planung konnte nicht starten." (`manager.solverNotStarted`, 9 Sprachen), statt still weiterzumachen. Kopfkommentar der Function an Punkt 32 angeglichen (HC-2 pro `mitarbeiter_id`, HC-5 Kalendermonat). Keine Schemaänderung. **Deployed am 2026-09-15 als Version 13** (per CLI aus dem Repo, `verify_jwt` an) — und damit zugleich die Solver-Änderung aus 1.1.5, die bis dahin nicht live war (Version 12 stammte vom 20.08.). Rauchtest ohne Nebenwirkung: ohne Token 401, ohne Zyklus-Id 400, unbekannter Zyklus 404. Sperrzeit und Anspruch selbst sind live **nicht** durchgespielt (bräuchte einen Chef-Token und einen echten Solver-Lauf). — `supabase/functions/plan-generieren/index.ts`, `src/app/(tabs)/manager.tsx`, `src/i18n/locales/*.json`

### 1.1.19
- **Notfallgrund: keine Aufforderung zu Gesundheitsangaben mehr, App bereit für `notfall_gruende`.** (a) Der Platzhalter im Notfall-Formular schlug in allen 9 Sprachen „Ich bin krank." vor und lud damit zu einem Gesundheitsdatum (Art. 9 DSGVO) ein, das niemand braucht. Jetzt neutral ohne Beispiel: „Kurze Begründung (keine Angaben zur Gesundheit)" bzw. „Short reason (no health details)" und Entsprechungen (Planung-Tab und Schichtdetail nutzen denselben Schlüssel `scheduling.reasonPlaceholder`). (b) `manager.tsx` liest den Grund nicht mehr aus `notfaelle.grund`, sondern aus der Tabelle `notfall_gruende` (engere RLS: Leitung, Melder, Übernehmer), die die vorbereitete Migration `QuickTeamFront/docs/backend/migration-2026-09-14-notfallgrund-tabelle.sql` anlegt. **Übergangsweise:** solange die Tabelle nicht existiert, liest die App den Grund wie bisher aus `notfaelle` — dieser Build läuft damit auf beiden Schemaständen und kann **vor** der Migration ausgeliefert werden (sonst bräche die Migration mit `drop column grund` den Manager-Screen älterer Builds). Den Rückfall entfernen, sobald die Migration live ist. — `src/app/(tabs)/manager.tsx`, `src/i18n/locales/*.json`

### 1.1.20
- **Kaputte Sonderzeichen im Manager-Bereich.** `manager.tsx` enthielt 22 doppelt kodierte Zeichen (UTF-8 als Windows-1252 gelesen, seit 2026-08-09) — sichtbar für Chefs z. B. als `â€”` statt „—" bei unbekanntem Melder, `â€œ…â€` statt Anführungszeichen um Urlaubskommentar und Notfallgrund, `Â·` statt „·" in Zeit-/Rollenzeilen, `â€¦`/`âœ“` statt „…"/„✓" am Speichern-Knopf. Zurückgewandelt; keine anderen Dateien betroffen (geprüft über `src/`). — `src/app/(tabs)/manager.tsx`

### 1.1.21
- **Kenntnisnahme der Datenschutzerklärung serverseitig nachweisbar; neue Fassungen sperren wieder** (Entscheidung der Betreiberin 2026-09-14). (a) **Sperre:** Eine neue Fassung von Datenschutzerklärung oder AGB zeigt jetzt dasselbe blockierende Fenster wie der erste Start — nur mit den geänderten Dokumenten im Volltext, Knopf „Zur Kenntnis genommen" erst nach dem Scrollen, kein Schließen, keine Zurück-Taste: zur Kenntnis nehmen oder die App nicht nutzen. Kehrt 1.1.14 um (Hinweis mit Ein-Tipp-Schließen); `legal.noticeDismiss` entfernt, `legal.noticeTitle`/`noticeBody` jetzt in allen 9 Sprachen. Die Kenntnisnahme der AGB in der App ist weiterhin **keine** Annahme durch den Betrieb (die erklärt ein Chef auf der Website). (b) **Nachweis:** Beim Betreten eines Betriebs schreibt die App eine Zeile nach `rechtliche_zustimmungen` — `dokument = 'datenschutz'` (Art `persoenlich`), aktuelle Fassung, `sprache` (Sprache des angezeigten Textes, beim Bestätigen unter `legal:acceptedLang` gespeichert; `de` oder `en`), `inhalt_hash`. Nur wenn auf dem Gerät genau die aktuelle Fassung zur Kenntnis genommen wurde. Eine Zeile je Betrieb und Person (die Tabelle verlangt `betrieb_id`, das Fenster erscheint aber vor der Anmeldung). Idempotent über den eindeutigen Index + `ON CONFLICT DO NOTHING`; läuft bei jedem Betreten, ein Fehlschlag (offline, Serverfehler) wird nur geloggt und beim nächsten Betreten erneut versucht, sperrt nie. Bestehende Nutzer, die die aktuelle Fassung schon bestätigt haben, bekommen ihre Zeile beim nächsten Betreten. AGB/AVV schreibt die App nicht (betrieblich, Sache der Website; die Policy würde Nicht-Chefs ohnehin abweisen). (c) **`inhalt_hash` ist der Wert der Website**, nicht ein Hash der App-Kopie: sha256 über die deutsche Markdown-Datei mit LF-Zeilenenden, wie `QuickTeamFront/src/lib/rechtstexte-inhalt.ts` — die App zeigt denselben Text ohne Markdown und käme sonst auf einen anderen Wert. Als Konstante `PRIVACY_POLICY_HASH` neben der Fassung, mit Befehl zum Neuberechnen; bei jeder neuen Fassung mitziehen. Geprüft: Hash aus Website-Datei und `legals/`-Kopie identisch (`ca3a9149…adbb2`); Insert unter RLS als angemeldetes Sandbox-Mitglied in zurückgerollter Transaktion → erste Zeile geschrieben, Wiederholung 0 Zeilen, nichts zurückgeblieben. Das Archiv-Trigger `private.archiviere_zustimmung` übernimmt `datenschutz`-Zeilen nicht (nur AGB/AVV), es entstehen also keine zusätzlichen Namens-/E-Mail-Kopien. Keine Schemaänderung. — `src/components/LegalConsentGate.tsx`, `src/lib/consentRecord.ts` (neu), `src/lib/legal.ts`, `src/lib/privacyPolicy.ts`, `src/lib/terms.ts`, `src/app/(tabs)/_layout.tsx`, `src/i18n/locales/*.json`

### 1.1.22
- **Launch-Checkliste Punkt 7 abgeschlossen — Edge Function `einladung-einloesen` gelöscht** (2026-09-15, per CLI). Sie war seit dem 2026-09-11 nur noch ein Grabstein (antwortete 410, stellte keine Sitzung mehr aus) und hatte keinen Aufrufer in App oder Website; der gültige Einladungsweg bleibt `meine_einladungen()` → `einladung_annehmen()`. Der Originalcode liegt archiviert in `QuickTeamFront/docs/audit-a/archiv/einladung-einloesen.index.ts.txt` und darf nicht unverändert zurück. Live bleiben `plan-generieren` (v13) und `push-versenden` (v4). Kein App-Code betroffen.
